...
export class CategoriesHeaderComponent {
  ...
  onAddCategoryClick = output<void>();
  ...
  async exportToPDF() {
    ...
    doc.text('Categories Report', 40, 45);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9.5);
    doc.setTextColor(...C.emerald600);
    doc.text('Enterprise Inventory Management System', 40, 62);
    ...
    const emptyCategories = categories.filter((c) => c.productsCount === 0).length;
    ...
    doc.setTextColor(...C.slate900);
    doc.text('All Categories', 50, tableStartY + 11);
    ...
    // 4. TABLE
    // ==========================================
    const head = [['#', 'Category Name', 'Description', 'Products', 'Status']];
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        1: { cellWidth: 120, fontStyle: 'bold' }, // Name
        2: {}, // Description
      ...
      didDrawPage: (data) => {
        ...
        if (logoBase64) {
          ...
          doc.setFontSize(8);
          doc.setTextColor(...C.slate500);
          doc.text('Enterprise Categories Report', 115, H - 19);
        } else {
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(8.5);
          doc.setTextColor(...C.emerald600);
          ...
          doc.setFontSize(7);
          doc.setTextColor(...C.slate500);
          doc.text('Enterprise Categories Report', 40, H - 11);
        }
    ...
    doc.save(`zaraa-categories-${new Date().toISOString().slice(0, 10)}.pdf`);
  }
}
